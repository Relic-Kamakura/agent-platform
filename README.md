...
1. fork するか個人ブランチを切る。ハンズオンでは本体 `07-full-app/` のコードを
   直接改造するので、共有の main を汚さない自分専用の作業場所を先に作る
...
- 00 → 06 は番号順に進める（03 で作ったツールを 06 でテストする依存がある）
- 08・09 は 03 まで終えていれば、04〜06 と並行して進められる。
  10 は 01 まで終えていれば他の章と独立に進められる
- 11 は 09 を終えてから、12 は 11 を終えてから進める
- 13 → 14 はこの順に続けて進める（14 の耐性 eval は 13 で作る評価ハーネス上で
  実行するため）。15〜19 はどの順で進めてもよい
