...
1 体のエージェントにすべてをやらせる代わりに、役割ごとの専門エージェントに分けて連携させる構成です。
役割を分けると、それぞれのシステムプロンプトを短く保て、役割に見合ったモデルを個別に選べるようになります。

### 5.1.2 分割の判断基準

1 体で足りる処理を分割すると、遅くなり、高くつき、デバッグ対象が増えるだけです。
「マルチエージェントにしたい」が目的化した案件で「この規模なら 1 体で十分です」と言えることも技術のうちです。
...
- ReviewAgent — 出典欠落や推測混入の検出。見逃しがそのまま報告の信頼性を落とすので、実案件では上位モデルの候補

役割ごとに理由を付けてモデルを選ぶ。これがコスト最適化の実体です。
Haiku（入力 $1 / 出力 $5、100万トークンあたり）と Sonnet（$3 / $15）の単価差は 3 倍あるので、理由なく全役割に上位モデルを配ると費用はそれだけで数倍になります。
この教材の既定は学習コストを抑えるため全役割とも Haiku ですが、役割ごとにモデル ID の設定を分けてあるのは、実案件で判断の役割だけを上位モデルに差し替えるためです。

### 5.2.2 agents-as-tools

Strands では、エージェントを丸ごと `@tool` で包むと、別のエージェントから見えるただのツールになります。
...
- 修正回数もコードで 1 回に固定。「revise が出る限り修正」にすると、コストとターン数の上限が読めなくなる

同じ理屈が細部にも効いています。ReviewAgent の判定行（VERDICT）がパースできなかったとき、この構成では「問題なし」ではなく revise と同じ扱いにします。パースに失敗しただけで、報告が検証されないまま通る事故を防ぐためです。
...
`exercises/tool_call_limiter.py` は第4章で作った CostLimiter の回数版で、これも編集不要です。ガードなしのエージェントを新設しないための部品として、組み立てるエージェントの hooks に付けます。
...
先に判定テスト `verify/test_specialist.py` を読むのも近道です。

### 5.3.2 tool_spec を確認する
...
compare_pricing の name と description と inputSchema が JSON で表示されるはずです。description の中身は、いま自分が書いた docstring です。
オーケストレータのモデルに渡るのはこの JSON だけで、中でエージェントが動いていることはどこにも書かれていません。エージェントがツールに見える、とはこのことです。
...
`5 passed` で合格です。
@tool で包まれていることとツール名、docstring の 3 節、専門エージェントの結線、ガードの発動を検査します。モデルは呼びません。
...
2 体のエージェントが動いていますが、オーケストレータに見えていたのは 5.3.2 で表示した JSON だけです。

## 5.4 まとめ

マルチエージェントの設計判断は、分けるかどうかよりも「分けた後にどう呼ぶか」に表れます。
スキップされたら困る工程はコードで固定し、依頼ごとに最適解が変わる工程だけをモデルの裁量に残す。**裁量とコードの境界を理由付きで引けること**が、この構成を案件で説明できるかどうかの分かれ目です。

エージェント同士の結線が壊れていないことは、人手で確かめ続けられません。
この章の verify がモデルを呼ばずに結線を検査したように、次章ではエージェントのテストの書き方そのものを身につけます。
