...
分割ありきで構成を決めず、まず 1 体で足りるかを確かめます。
...
この教材の既定は学習コストを抑えるため全役割とも Haiku ですが、役割ごとにモデル ID の設定を分けてあるのは、判断の役割だけを上位モデルに差し替えられるようにするためです。
...
同じ判断を細部にも適用しています。ReviewAgent の判定行（VERDICT）がパースできなかったとき、この構成では「問題なし」ではなく revise と同じ扱いにします。パースに失敗しただけで、報告が検証されないまま通る事故を防ぐためです。
...
スキップされたら困る工程はコードで固定し、依頼ごとに最適解が変わる工程だけをモデルの裁量に残す。**裁量とコードの境界を理由付きで引けること**が、この章の到達点です。
