...
だからツールの出来は、コードそのものより先に、この 2 つの文字列で決まります。

この 2 つが雑だと、エージェントは静かに壊れます。
説明が曖昧だと、モデルはツールを誤用します。失敗を空文字で隠せば、推測で埋めた報告が上がってきます。本文を丸ごと返せば、毎ターンの入力が膨らんで費用が増える。
どれも例外では落ちないので気づきにくい壊れ方で、この章の規約はこれを防ぐためにあります。
...
graph TD
    ...
    J -->|"「含まないもの」に該当<br/>（JS ページ・認証つき等）"| A["使わずに別の手段を選ぶか、<br/>「確認できず」と報告する"]
    U --> E["コードが実行"]
...
ログには何も残らず、開発者は品質劣化の原因を追えません。二重に有害です。
...
# このリポジトリの型でモデルが受け取るもの（format_tool_error の実際の出力）
...
この型を作る `format_tool_error` は、骨組み `exercises/fetch_page.py` に用意してあります:

- 例外に `retryable`（再試行に意味があるか）と `hint`（次に取るべき行動）を持たせる
- ツール層で `format_tool_error()` により `ERROR[...]` 形式の文字列にしてモデルへ返す
- retryable: no と next_action を読んだモデルは、推測で埋める代わりに「確認できず」と報告する方向へ切り替わる

### 3.2.4 リトライの範囲

外部 API の呼び出しは、相手の混雑や一時障害でときどき失敗します。
ただし失敗には「時間を置けば直る可能性があるもの」と「何度呼んでも同じもの」があり、区別せず全部リトライすると、直らない失敗のために待ち時間と費用を無駄にします。
だから失敗の種類で行動を分けます。この章で作る fetch_page の方針は次の 2 行です。

| 失敗 | 意味 | fetch_page の行動 |
| --- | --- | --- |
| timeout / 5xx | 相手の混雑・一時障害。時間を置けば直る可能性がある | リトライする |
| 403 / 404 などの 4xx | こちらの依頼が誤っている。何度呼んでも結果は同じ | リトライせず、即 `ERROR[...]` を返す |

リトライの間隔は指数バックオフ（1 秒 → 2 秒 → 4 秒と倍にしていく）にします。
毎回同じ短い間隔で叩き直すと、相手が復旧する前に試行を使い切りやすく、障害中の相手に負荷をかけ続けることにもなるためです。
...
返す量の設計もツールの責務です。
生 HTML を丸ごと返すと、その全文が以降の毎ターンの入力に含まれ続け、トークン費用が増え続けます。
...
先に判定テスト `verify/test_fetch_page.py` を読むのも近道です。
要求仕様そのものになっています。
...
成功側は「Example Domain」を含む生 HTML の先頭 200 文字が表示され（HTML の整形はこのツールの「含まないもの」です）、失敗側は 3.2.3 で見た 3 行（`ERROR[PageFetchError]: ...` / `retryable: no` / `next_action: ...`）が表示されるはずです。
...
モデルが toolUse で fetch_page を選び、取得した本文をもとに 1 行の要約が返るはずです。
