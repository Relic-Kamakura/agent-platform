...
if jq -e '.context.logLevel' cdk.json >/dev/null 2>&1; then
  ...
  ng 'cdk.json の context に "logLevel" を追加してください（README 17.3.2）'
fi
...
if echo "$SYNTH" | grep -q "LOG_LEVEL: DEBUG"; then
  ...
  ng "synth 結果に LOG_LEVEL: DEBUG がありません。lib/config.ts の agentEnvironment を確認してください（README 17.3.1）"
fi
...
if echo "$SYNTH_DEFAULT" | grep -q "LOG_LEVEL: INFO"; then
  ...
  ng "既定（cdk.json の logLevel=INFO）が synth に反映されていません（README 17.3.2）"
fi
