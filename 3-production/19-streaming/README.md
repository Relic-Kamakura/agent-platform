...
画面のためだけではありません。
AgentCore Runtime のタイムアウトは同期よりストリーミングの方が長く取られています（第16章、値は versions.md）。
...
API の認可に使うのはアクセストークンです（第18章 18.1.3）。
...
## 19.3 ハンズオン: Route Handler で JWT を検証する
...
```bash
cd 3-production/19-streaming && npm run dev
```
...
第18章のデプロイ後、`.env.local` を本番向けに切り替えます。
`AUTH_BYPASS=false` にし、`LOCAL_AGENT_URL` を消し、`AGENT_RUNTIME_ARN` と `COGNITO_*` を CfnOutput の値で埋めます。
第18章 18.4 で取得したアクセストークンを `Authorization` ヘッダに付けて呼び出し、トークン無しが 401 になることも確認してください。
...
次の第9章では、この画面に流れてくる報告の品質そのものを evals で測ります。
