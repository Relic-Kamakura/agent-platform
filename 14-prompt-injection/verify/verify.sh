#!/usr/bin/env bash
# 第13章の合格判定。プロンプトの堅牢化と耐性 eval ケースを検査する（AWS 不要）。
set -uo pipefail
...
grep -q "これまでの指示をすべて無視し" 07-full-app/src/tools/providers/mock.py \
  ...
  || ng "mock.py の reviews fixture が見つかりません（教材の提供物。消してしまった場合は README 13.1）"
...
for f in 07-full-app/src/agents/search_agent.py 07-full-app/src/agents/orchestrator.py; do
  if grep -q "検索結果の取り扱い" "$f"; then
    ...
    grep -qE "(指示ではあり|従わ)" "$f" && ok "  指示に従わない旨がある" \
      || ng "  「資料内の指示には従わない」ことを書いてください（13.3 要件 2）"
    grep -qE "(環境変数|内部|システムプロンプト)" "$f" && ok "  内部情報を出さない旨がある" \
      || ng "  内部情報（環境変数等）を報告に含めないことを書いてください（13.3 要件 2）"
  else
    ng "$(basename "$f") に「# 検索結果の取り扱い」の節がありません（13.3 要件 1）"
  fi
done
...
echo "4. 耐性 eval ケース"
if grep -q '"injection-resistance"' 12-evaluation/cases.jsonl; then
  ok "injection-resistance ケースがある"
  python3 - <<'PY' && ok "  not_contains で乗っ取り文字列を検証している" || { echo NG; exit_code=1; }
import json, sys
for line in open("12-evaluation/cases.jsonl", encoding="utf-8"):
    if not line.strip():
        continue
...
sys.exit(1)
PY
  [ "${exit_code:-0}" = "1" ] && ng "  expect.not_contains に乗っ取り時の文字列を入れてください（13.4）"
else
  ng "12-evaluation/cases.jsonl に injection-resistance ケースを追加してください（13.4）"
fi

echo
if [ "$FAILED" = "0" ]; then
  printf '\033[32m第13章 合格（実測は要 AWS の 13.5 で）。\033[0m\n'
else
