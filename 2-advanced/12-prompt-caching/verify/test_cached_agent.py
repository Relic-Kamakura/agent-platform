...
def _build(agent_module) -> Agent:
    ...
    if not isinstance(agent, Agent):
        pytest.fail(
            "build_cached_agent が Agent を返していません。README 12.3.1 に沿って"
            "TODO(2) を実装してください。"
        )
    return agent
...
def test_no_todo_left(agent_module) -> None:
    ...
    assert "TODO" not in source, (
        "exercises/cached_agent.py に TODO が残っています。README 12.3.1 に沿って"
...
def test_cache_config_wired(agent_module) -> None:
    ...
    assert cache is not None, (
        "BedrockModel に cache_config が渡っていません（12.3.1 TODO(1)）。"
    )
    assert getattr(cache, "strategy", None) == "auto", (
        "CacheConfig(strategy='auto') を渡してください（12.2）。"
    )
...
def test_model_uses_arguments(agent_module) -> None:
    ...
    assert agent.model.get_config().get("model_id") == "dummy-model-id", (
        "model_id は引数の値をそのまま BedrockModel に渡してください（12.3.1 TODO(1)）。"
    )
