...
def test_no_todo_left(prompt_module) -> None:
    ...
    assert "TODO" not in source, (
        "exercises/hardened_prompt.py に TODO が残っています。README 10.3 に沿って"
...
def test_refuses_and_reports(prompt_module) -> None:
    ...
    assert re.search(r"指示では(ない|あり)|指示として扱わ", prompt), (
        "検索結果は資料（データ）であり指示ではない、と書いてください（10.3 要件 1）。"
    )
    assert "従わ" in prompt, (
        "資料内の指示めいた文章には従わない、と書いてください（10.3 要件 2）。"
    )
    assert "報告" in prompt, (
        "従わないだけでなく、不審な指示があった事実を報告させてください（10.3 要件 2）。"
    )


def test_keeps_internal_info(prompt_module) -> None:
    assert SECTION_HEADING in prompt_module.HARDENED_PROMPT, (
        f"先に「{SECTION_HEADING}」の節を書いてください（10.3.2）。"
    )
