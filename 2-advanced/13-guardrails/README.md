...
PROMPT_ATTACK フィルタは第10章と直接つながります。
...
第10章のプロンプト層が要る理由はここにあり、Guardrail が受け持つのは利用者入力側の攻撃（ジェイルブレイク等）と、モデルが出力する内容の側です。
...
`trace` を有効にすると、どのポリシーが何を遮断したかがレスポンスに含まれるので、13.5 の発動確認ではこれを見ます。
...
3. `CfnOutput` で GuardrailId と GuardrailVersionNumber を出力する。13.5 で環境変数に入れる値です
...
1. `guardrail_id` と `guardrail_version` の両方が指定されているときは、それを渡した `BedrockModel` を返す（13.2.2 のコードの形）
...
13.3 の Guardrail をデプロイし、13.4 のモデルに接続して、プロンプト攻撃調の入力を送ります。
...
どちらか片方で足りるものではなく、第10章のプロンプト側防御も含めた多層防御の一層ずつという分担です。
発動の確認（13.5）を終えたら、次は取り消せない操作に人間の承認を挟むゲートを作ります。
