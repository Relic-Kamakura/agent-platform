#!/usr/bin/env bash
# 第13章（CDK 側）の合格判定。型チェックと synth の結果を検査する。
...
if [ ! -d node_modules ]; then
  ng "依存が入っていません。13-guardrails で npm ci を実行してください"
  exit 1
fi
if [ ! -f lib/guardrail-stack.ts ]; then
  ng "lib/guardrail-stack.ts がありません。exercises/guardrail-stack.ts をコピーして TODO を埋めてください（13.3.1）"
  exit 1
fi
if grep -q "TODO" lib/guardrail-stack.ts; then
  ng "lib/guardrail-stack.ts に TODO が残っています。README 13.3 に沿って実装し、終わったら TODO コメントを消してください"
  exit 1
fi
...
echo
if [ "$FAILED" = "0" ]; then
  printf '\033[32m第13章（CDK 側）合格。アプリ側は uv run pytest -q で判定してください。\033[0m\n'
else
