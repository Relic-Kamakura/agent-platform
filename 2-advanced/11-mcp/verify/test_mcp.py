"""第11章の合格判定。サーバ起動からツール実行まで、プロトコル越しに検証する（LLM なし）。"""
...
def test_no_todo_left() -> None:
    ...
    assert "TODO" not in source, (
        "exercises/server.py に TODO が残っています。README 11.3 に沿って実装し、"
...
def test_server_exposes_company_search(mcp_client) -> None:
    ...
    assert "company_search" in names, (
        ...
        "README 11.3 の TODO(1) を実装してください。"
    )
...
def test_tool_call_over_protocol(mcp_client) -> None:
    ...
    assert "49" in text, (
        ...
        "大文字小文字を無視して _DATA を引いてください（README 11.3 の TODO(2)）。"
    )
