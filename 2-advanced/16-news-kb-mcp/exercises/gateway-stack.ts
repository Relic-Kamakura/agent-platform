// ハンズオン 16.5: AgentCore Gateway の読み取り経路。
// Cognito とツール Lambda は完成済み。
...
export class NewsGatewayStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props: NewsGatewayStackProps) {
    ...
    // Cognito（完成済み）。ユーザープールとアプリクライアントが JWT を発行し、
    // 下の Gateway がその JWT を検証する
    ...
    // Gateway の実行ロール（完成済み）。aws:SourceAccount を条件に付け、
    // 他アカウントの Gateway からこのロールを使わせない
    ...
    //     discoveryUrl で公開鍵の場所を、allowedClients で宛先を検証する
    ...
    //   ツール定義の description は「受け取るもの / 返すもの / 含まないもの」の 3 節で書く。
    //   これがモデルにとってのツールの仕様書になる。
    ...
    // TODO(3): CfnOutput を 3 つ出す。16.6 の接続で使う。
