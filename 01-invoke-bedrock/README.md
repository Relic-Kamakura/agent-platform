...
`modelId` の差し替えだけで済むのは、この層があるためです。
...
コンテキストウィンドウは入力と出力を合わせた総枠で、最大出力はそのうち出力側だけに掛かる上限です。
どちらもモデルごとに違い、この教材の既定モデルの値は versions.md を参照してください。

総枠を超えたときは呼び出し自体が例外になるので、すぐ気づけます。
注意が要るのは出力側で、`maxTokens` に達しても例外は出ません。
...
単価はモデルとリージョンごとに決まり、出力は入力より数倍高くなっています（versions.md を参照）。
...
コストを決めるのは呼び出し回数だけでなく、何を入れて何を出させるかです。
...
非同期（`start_async_invoke`）は結果を後から S3 で受け取る方式で、動画生成など長時間処理向けのため、Claude のテキスト生成では使いません。
...
自動ルーティングされます。追加料金はなく、課金は呼び出し元リージョンの単価です。
...
`us.` を選べば米国内のいずれかのリージョンで推論が実行され、
宛先の一覧は AWS が決めるためこちらでは選べません。
データを国外に出せない案件では、ここが採否を決めます。
国内に閉じたい場合は国別の `jp.` プロファイルを使いますが、対応するモデルは限られます。
使いたいモデルに `jp.` があるかは、1.3 の一覧コマンドで確認してください。
...
ID を間違えると `ValidationException` が返りますが、接頭辞が違うのかモデルが無いのかは
例外からは区別できません。そのためコードを書く前に、自分のリージョンで呼べる ID の一覧を
確認します。
...
料金が 1 桁変わることを確認してください。入力より単価の高い出力を
...
first_token はその何分の一かです。この差がそのまま体感の差になります。
...
Converse API は、モデルが違っても `messages` と `inferenceConfig` と `usage` の
3 つで呼べるようにする層です。1 回の料金は `usage` の入出力トークン数から計算でき、
1.5 で見たとおり金額を決めるのは出力の長さです。同期とストリーミングの差は
最初の 1 文字が出るまでの時間に現れ、1.6 の first_token と total の開きがそれです。

呼ぶ前に ID の実在を確認し、呼んだ後に usage を見る。この 2 つが、
この先すべての章のモデル呼び出しの前提になります。エージェント(第2章)も、
この Converse API の繰り返しです。
