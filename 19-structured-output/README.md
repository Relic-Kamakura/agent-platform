...
revise に倒す」で安全側に защってはいるものの、本来 ok の報告が revise 扱いに
...
第3章の「docstring は仕様書」と同じ原理です。自然文の約束をスキーマに
格上げする、という一手。

## 18.3 【ハンズオン】ReviewAgent を書き換える
...
4. 構造化結果から `ReviewOutcome` への変換を、テスト可能な純粋関数
   ...
   - `None`（構造化出力が得られなかった場合）は **revise に倒す**。
...
```bash
uv run --project 07-full-app pytest 18-structured-output/verify -q
```
...
第12章の eval を流し、退行が無いことを確認してください。

```bash
uv run --project 07-full-app python 12-evaluation/run_eval.py
```

ついでに `token_usage` ログの review ロールを前後比較すると、形式指定の
プロンプトが消えた分と、修正パスの空振りが減った分の差が見えることがあります。
