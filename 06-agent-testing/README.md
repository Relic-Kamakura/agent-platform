...
ハンズオンの対象は、第3章で作った `fetch_page` の完成品（`target/` に同梱）。テストの雛形 1 本を手がかりに、残り 6 本を書くところまでが課題です。
...
このリポジトリではリトライ回数まで assert します（初回 + 2 回 = ちょうど 3 回で打ち切る）。`time.sleep` も差し替えるので、指数バックオフのテストでも待ち時間はゼロです。
...
書くのは `exercises/test_fetch_page.py` で、第3章の verify を写すのではなく、要件の観点ごとに自分で書きます。

要件は 3 つです。

1. テスト関数を 7 つ以上（第3章の verify と同じ本数）
2. 次の観点を含める
...
モック部品（`_Response` と `_client`）と正常系のテスト 1 本は書いてあり、TODO が 5 つ残っています。

1. URL 検証。`ftp://` などを渡し、httpx をモックせずに `ERROR[` 形式が返ることを assert する
2. 4xx。404 を返すモックで、`ERROR[` 形式と試行が 1 回で止まることを assert する
3. リトライ回数。タイムアウトを起こし続け、回数カウンタで「初回 + リトライ 2 回 = 3 回」を assert する
4. 5xx。503 を 2 回返したあと 200 を返すモックで、本文が返ることを assert する
5. `target/fetch_page.py` を読み、上記でテストされていない分岐を 2 つ見つけてテストを書く。合計 7 本以上にする
...
verify が見るのは本数と手法だけで、テストの内容の妥当性（何を assert しているか）は判定しません。
...
全 7 本は `solutions/test_fetch_page.py` にあります。
