...
if [ ! -f lib/guardrail-stack.ts ]; then
  ng "lib/guardrail-stack.ts がありません（README 17.3）"
else
  ...
  echo "$SYNTH" | grep -q "PROMPT_ATTACK" && ok "PROMPT_ATTACK フィルタがある" \
    || ng "contentPolicyConfig に PROMPT_ATTACK フィルタを入れてください（17.3 要件 1）"
  echo "$SYNTH" | grep -q "AWS::Bedrock::GuardrailVersion" && ok "版を発行している" \
    || ng "CfnGuardrailVersion で版を発行してください（17.3 要件 2）"
fi

echo "2. アプリ側の配線"
cd "$REPO_ROOT"
uv run --project 07-full-app python - <<'PY' && ok "guardrail 設定がモデルに渡る" || ng "config/models の配線が未完です（17.3 要件 5-6）"
import os, sys
...
echo
[ "$FAILED" = "0" ] && printf '\033[32m第17章 合格。\033[0m\n' || printf '\033[31m未達の項目があります。\033[0m\n'
exit "$FAILED"
