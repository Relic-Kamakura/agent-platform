...
Bedrock Knowledge Bases はこれをマネージドで肩代わりする機能です（第1章 1.1.4 の位置づけ表を参照）。
...
先に判定テスト `verify/test_mini_rag.py` を読むのも近道です。
要求仕様そのものになっています。
...
だからツール設計の規約（第3章）も、検索結果を信頼しない原則（第14章）も、そのまま適用されます。
