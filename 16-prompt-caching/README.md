# 第16章 プロンプトキャッシュ

この章を終えると、プロンプトキャッシュを設定で有効化でき、どんなプロンプト構造なら
キャッシュが再利用されるのか（されないのか）を説明できるようになります。
実装は 07-full-app への追加です。

## 16.1 概要

### 16.1.1 何を解決するか

エージェントはループのたびに、システムプロンプト・ツール定義・会話履歴の全体を
モデルへ再送します（API はステートレス。第2章）。つまりターンが進むほど、同じ前半部分を
毎回フルコストで処理し直すことになります。この重複処理を減らすのがプロンプトキャッシュで、
前回と重なる部分の処理結果をモデル側が再利用し、その分の課金を下げる仕組みです。

### 16.1.2 前方一致の仕組み

仕組みは前方一致です。リクエストの先頭からの並びが前回と一致した部分は、
モデル側が処理済みの状態を再利用し、大幅に安い単価で課金されます。
裏を返すと、**先頭に 1 バイトでも差が入ると、それ以降は全部キャッシュミス**です。

エージェントと相性が良い理由も構造から分かります。ループの各ターンは
「前回までの内容 + 新しいツール結果」なので、前半は毎回同じ。
まさに前方一致でキャッシュが再利用される形をしています。

キャッシュが再利用されなくなる典型例も知っておいてください。

- システムプロンプトに現在時刻やリクエスト ID を埋め込む（毎回先頭が変わる）
- ツール定義の順序が実行のたびに変わる（set から生成している等）
- ターンの途中でモデルやツール構成を切り替える

## 16.2 実装のポイント

Strands の `BedrockModel` は `cache_config` を受け取ります（実機で確認済み）。

- `CacheConfig(strategy="auto")` — キャッシュポイントの配置を自動で行う。まずこれ
- `CacheConfig(strategy="anthropic", ttl=...)` — Anthropic 方式の明示配置。細かく制御したいとき

07-full-app のシステムプロンプトが固定文字列で、動的な値を含まないのは、
このキャッシュを見据えた設計でもありました。現在時刻を埋め込んだ時点で、16.1.2 の
「先頭 1 バイトの差」に該当し、全ターンがキャッシュミスになります。

## 16.3 【ハンズオン】設定で切り替えられるようにする

規約どおり、有効/無効は `.env` で切り替えられるようにします。

1. `07-full-app/src/config.py` の Settings に `enable_prompt_cache: bool = False` を追加する。
   既定 False の根拠コメントを書くこと
   （ヒント: キャッシュ書き込みには割増があり、1 往復で終わる開発中の試行では
   逆に高くつくことがある。効果はターン数が伸びるほど出る）
2. `07-full-app/src/agents/models.py` の `build_model()` で、有効時のみ
   `cache_config=CacheConfig(strategy="auto")` を `BedrockModel` に渡す
3. `.env.example` に `ENABLE_PROMPT_CACHE=false` を根拠コメント付きで追記する
4. 既存テストを壊さない

判定を流します。

```bash
uv run --project 07-full-app pytest 16-prompt-caching/verify -q
```

`3 passed` で合格です。

## 16.4 【ハンズオン】効果を実測する

第13章の eval にコスト集計があるので、前後比較がそのまま計測になります。

```bash
ENABLE_PROMPT_CACHE=false uv run --project 07-full-app python 13-evaluation/run_eval.py
```

```bash
ENABLE_PROMPT_CACHE=true uv run --project 07-full-app python 13-evaluation/run_eval.py
```

トークン数の内訳と所要時間を比べてください。マルチエージェント構成（第5章）は
エージェントごとにシステムプロンプトが違うため、キャッシュはエージェント単位で
別々に作られ、それぞれのループ内で再利用されます。単価の詳細は Bedrock の料金表で確認すること（モデルごとに
キャッシュ読み取り・書き込みの係数が違うため、ここには書かない）。

## 16.5 まとめ

プロンプトキャッシュは **「先頭からの並びが前回と一致した部分だけ安くなる」** という
単純な規則で動き、再利用されるかどうかはプロンプトの構造で決まります。だから有効化は
`cache_config` の 1 行でも、システムプロンプトを固定文字列に保つ設計判断のほうが
本体です。

## 次の章

[第17章 Bedrock Guardrails](../17-guardrails/)
