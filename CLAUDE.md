...
- 章の一覧と進め方: @README.md（部ごとの入口は 1-basic/ 2-advanced/ 3-production/ の README）
...
- `1-basic/NN-*/`・`2-advanced/NN-*/`・`3-production/NN-*/` — 学習の章。各章は README（章番号付き節 + ハンズオン節）+ exercises/（TODO 付き骨組み）+
...
- 応用編の章は knowledge-base / evaluation / prompt-injection / mcp /
  ...
  本番運用基盤の章は agentcore-deploy / infra-as-code / auth / streaming（16〜19）
...
- **例外を握りつぶさない。** ツール失敗は retryable と次の行動をエージェントに返す。
  例外は `1-basic/07-full-app/src/errors.py` に定義する。
...
- 章の節の並びは固定: 冒頭（到達点）→ `## N.1 概要` → `## N.2 実装のポイント` →
  ...
  `1-basic/01-invoke-bedrock/README.md`、詳細は writing-style.md の「章の構成」。
