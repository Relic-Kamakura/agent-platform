...
外部の公開情報を引くのに対し、RAG は自分たちのデータを引く。
Bedrock Knowledge Bases は、S3 のドキュメント取り込み・ベクトル化・検索 API までを
マネージドで提供する AWS 版の RAG 基盤。
仕組みの基礎と最小の retrieve ツールは [第10章](../10-knowledge-base/) が本編で扱う。
この節は第10章の先、KB + S3 + ベクトル検索を組み合わせた本格構成に
進むときの入口として残す。

本編との接続は第3章のツール設計で、Retrieve API をツールとして包めば
このエージェントは社内文書も引けるようになる（第10章で実装する）。
入口は Bedrock Knowledge Bases の開発者ガイド。
本編の題材（Web 検索型）とは混ぜず、別モジュールとして作ること。
...
東京リージョンにも対応済み。
入口は AgentCore Memory の開発者ガイドと Strands の session_manager。
...
`opentelemetry-instrument` 経由で起動する（公式ドキュメントの手順）。
...
第15章の発展にあたる。自作 MCP サーバとの使い分けは、変換ロジックが要るなら自作、
既存 API をそのままツールとして公開するだけなら Gateway。
入口は AgentCore Gateway の開発者ガイド（東京リージョン対応済み）。

## E. 用語集

本編では扱わないが、案件の要件定義や資格の出題範囲で名前が出るもの。
何をするものかと、どういう要件のときに候補になるかだけ載せる。
...
Kendra は SharePoint や Salesforce のコネクタを持ち、利用者ごとに見える範囲を変えられる。
文書を S3 に集められるなら Knowledge Bases（第10章）で足りることが多い。
JumpStart を使うと、推論エンドポイントを常時起動する費用と運用を引き受けることになる。
AWS Artifact で取れるのは SOC や ISO の報告書で、セキュリティチェックシートの証跡に使う。
DataZone のデータリネージはデータの出どころと加工経路の追跡で、
RAG の取り込み元の統制（第10章）とつながる領域。
Model Monitor は学習時からのずれを見るもので、基盤モデルを API で呼ぶ構成では出番が無い。
その役目は第13章の evals と第4章のログが担う。
