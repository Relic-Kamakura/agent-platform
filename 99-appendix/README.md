...
ここはその先、本格構成に進むときの入口として残す。

- 本編との接続: 第3章のツール設計がそのまま使える。Knowledge Bases の Retrieve API を
  ツールとして包めば、このエージェントは社内文書も引けるようになる（第10章で実装する）
- 入口: Bedrock Knowledge Bases の開発者ガイド。KB + S3 + ベクトル検索の本格構成が対象
...
第15章で自作した MCP サーバの「運用をマネージドに寄せた版」と考えるとよい。

- 本編との接続: 第15章の発展。自作 MCP サーバとの使い分けは
