...
第2章で見たとおり、モデルに渡るのはツールの名前と説明と引数スキーマだけで、モデルはその文面から使いどころを推測します。
...
3 つ目の「含まないもの」がもっとも誤用を防ぎます。悪い例と良い例を並べます。
...
fetch_page の verify が通ったら第4章へ進んでください。今度はツール呼び出しに上限を掛けて、呼び出し回数とトークン消費をコードで制御します。
