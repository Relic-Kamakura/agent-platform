...
Orchestrator は観点への分解と統合を担い、判断を伴うので上位モデルに差し替える候補です。
SearchAgent はクエリ整形と事実抽出だけの定型処理で、軽量モデルで足ります。
ReviewAgent は出典欠落や推測混入を検出します。見逃しがそのまま報告の信頼性を落とすので、これも上位モデルに差し替える候補です。

コストの差は、役割ごとのモデル選択でつきます。
Haiku と Sonnet の単価差は 3 倍あるので（versions.md）、理由なく全役割に上位モデルを割り当てると費用はそれだけで数倍になります。
...
Strands にはほかに Graph と Swarm があります。
agents-as-tools は上位が判断しながら委任する形、Graph は手順が固定のパイプラインで、分岐や並列を宣言的に固定したいときに使います。
Swarm はエージェント同士の対等な協調で、コスト予測が難しいので最後の手段です。

### 5.2.3 どこをモデルに任せ、どこをコードで固定するか

SearchAgent はツールで、何回、どの観点で呼ぶかはモデルの裁量です。依頼ごとに最適な分解が違うので、固定しないことに LLM を使う価値があります。
ReviewAgent はコードが必ず 1 回実行します。ツールにすると、モデルが今回はレビュー不要と判断した時点で検証がスキップされます。
修正回数もコードで 1 回に固定しています。revise が出る限り修正する形にすると、コストとターン数の上限が決まらなくなります。
...
もう 1 つの基準は、コードで確実に求まる値をツールにしないことです。
AWS のブログは「来月の支出レポートを作成して」という依頼で、現在日付を `get_current_date()` ツールとして公開する構成と、コードで取得した日付を属性として渡す構成を比べています。
属性で渡す方が LLM 呼び出し回数、合計トークン数、レイテンシーのいずれも少ない結果です（値は versions.md）。
ツールにすると、日付を取るためだけに推論が 1 回増えるためです。
...
1. `build_specialist_agent` で専門エージェントを組み立てる。モデル、システムプロンプト、`lookup_pricing`、そして hooks に `ToolCallLimiter` を渡す
2. `compare_pricing` の docstring を第3章の 3 節構成で書く。「含まないもの」に価格以外（機能や評判）を調べないことを明記する
3. `compare_pricing` の中身で専門エージェントに依頼文を渡し、結果を文字列で返す
