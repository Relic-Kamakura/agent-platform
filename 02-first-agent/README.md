...
（`toolUseId` 付き）で返り、こちらは実行結果を `toolResult` ブロックとして次の
リクエストに積む。Strands の `Agent` はこの往復の実装で、1 往復を cycle と呼びます。

## 2.2 【ハンズオン】最小のエージェントを書く
...
## 2.3 いま書いたものを ReAct として読む

ReAct は Yao らの論文（2022、ICLR 2023 採録）で提案されたパターンです。
推論だけの CoT より、外部と相互作用しながら考える方が幻覚が減るというのが主張でした。
2.2 で書いたループがそのまま対応します。

| ReAct のステップ | 2.2 のコードでの実体 |
| --- | --- |
...
もうひとつ、名前だけ知っておくべき用語があります。CoT（Chain of Thought、
Wei らの論文 2022）は、複雑な問題を思考ステップに分解させるプロンプト技法です。
フレームワークの機能ではありません。

本体での実物は `07-full-app/src/agents/orchestrator.py` のシステムプロンプトの一文、
「依頼を 2〜4 個の調査観点に分解し、観点ごとに investigate を 1 回ずつ呼ぶ」。
この指示が無いと、モデルは 1 回の検索に全部を詰め込み、浅い調査で結論を出しがちです。
第5章で戻ってきます。
