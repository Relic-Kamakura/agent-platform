...
def test_01_agent_has_now_tool() -> None:
    ...
    assert hasattr(mod, "agent"), "01_agent.py にモジュールレベルの agent を定義してください（2.2）。"
    assert "now" in set(mod.agent.tool_names), "agent の tools に now を渡してください（2.2）。"
...
def test_01_agent_docstring_sections() -> None:
    ...
    for section in ("受け取るもの", "返すもの", "含まないもの"):
        assert section in doc, f"now の docstring に「{section}」がありません（2.2 の写経を確認）。"
...
def test_02_char_count_contract() -> None:
    ...
    doc = fn.__doc__ or ""
    for section in ("受け取るもの", "返すもの", "含まないもの"):
        assert section in doc, f"char_count の docstring に「{section}」がありません（2.4 要件 2）。"
...
def test_03_metrics_script() -> None:
    ...
    assert path.exists(), "03_metrics.py がありません（2.5）。"
